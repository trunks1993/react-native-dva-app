...
import iconHand from './images/icon-hand.png';
...
export async function getList(skillTypeId: number | string): Promise<any> {
  try {
    const response = await fetch(`http://192.168.0.119:8081/terminal/v1/skillLearning/selectLearningType?learningType=1&skillTypeId=${skillTypeId}`, {
      method: 'GET',
      headers: {
...
export async function getCategory(): Promise<any> {
  try {
    const response = await fetch('http://192.168.0.119:8081/terminal/v1/device/treeType/selectAll?treeType=4', {
      method: 'GET',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
...
export interface BookProps extends NavigationInjectedProps {
}
...
class Book extends React.Component<BookProps, BookStates> {
  state: BookStates = {
    list: [],
    categoryList: [],
    totalPrice: 0,
  ...
  getCategory: () => void = async () => {
    ...
    if (!err) {
      this.setState(produce((draft: Draft<BookStates>, { }): void => {
  ...
  render() {
        ...
        <ImageBackground source={shopBgItem} style={ShopStyles.productItem}>
          ...
          <View style={ShopStyles.productItemContent}>
            ...
            <View style={LearningStyles.stockBox}>
              <Text style={{ fontSize: 18, color: 'rgba(255,255,255,0.5)' }}>剩余:{item.stock}本</Text>
            </View>
            <TouchableOpacity onPress={() => {}}>
              <ImageBackground source={bgBtn} style={LearningStyles.bookItem}>
                <Image source={iconHand} />
                <Text style={{fontSize: 22, color: '#ffffff', marginLeft: 10}}>借阅</Text>
              </ImageBackground>
            </TouchableOpacity>
          </View>
        </ImageBackground>
    ...
    return (
      <ImageBackground source={bgPage} style={GlobalStyles.container}>
        <Header replace={navigation.replace} title="借阅书籍" allowBack={true} backPage="Learning" titleRight="0502监室  |  2019年12月3日 星期二    15:20" />
