...
import iconPlay2 from './images/icon-play2.png';
...
export interface VideoListProps extends NavigationInjectedProps {
}
...
export interface VideoListStates {
  list: ItemType[];
  totalPrice: number;
  carCount: number;
...
class VideoList extends React.Component<VideoListProps, VideoListStates> {
  state: VideoListStates = {
    list: [],
    categoryList: [],
    totalPrice: 0,
    carCount: 0,
  ...
  componentDidMount() {
    ...
    const skillTypeId = navigation.getParam('skillTypeId');
    this.getList(skillTypeId);
  }

  getCategory: () => void = async () => {
    let [err, data] = await getCategory();
    if (!err) {
      this.setState(produce((draft: Draft<VideoListStates>, { }): void => {
        draft.categoryList = data.data;
      }));
    }
  }

  getList: (skillTypeId: number) => void = async (skillTypeId) => {
    const [err, data] = await getList(skillTypeId);
    if (!err) {
      this.setState(produce((draft: Draft<VideoListStates>, { }): void => {
  ...
  render() {
    ...
    const product: React.FC<ItemType> = (item) => {
      ...
      return (
        <View style={LearningStyles.listItem}>
          <TouchableOpacity onPress={() => navigation.navigate('VideoPlay')} style={LearningStyles.listItemTouch}>
            <Image source={iconPlay2} />
    ...
    return (
      <ImageBackground source={bgPage} style={GlobalStyles.container}>
        <Header replace={navigation.replace} title="视频资料" allowBack={true} backPage="Video" titleRight="0502监室  |  2019年12月3日 星期二    15:20" />
        <ImageBackground source={shopBgContent} style={[ShopStyles.content, { marginTop: 36, paddingLeft: 64, paddingRight: 64 }]}>
          <View style={ShopStyles.product}>
            <FlatList data={list} keyExtractor={item => item.skillLearningId} renderItem={({ item }) => product(item)} />
          </View>
        </ImageBackground>
      </ImageBackground>
    )
